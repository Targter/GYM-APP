...
const FitnessApp = () => {
  ...
  const glassPanel =
    "backdrop-blur-lg bg-white/30 border border-white/40 shadow-lg";
  const gradientText =
    "bg-gradient-to-r from-purple-500 via-purple-500 to-sky-500 bg-clip-text text-transparent";
  const neumorphicInput =
    "bg-gray-50 border-2 border-transparent rounded-xl shadow-[4px_4px_10px_0px_rgba(0,0,0,0.1),-4px_-4px_10px_0px_rgba(255,255,255,0.9)] focus:shadow-[inset_4px_4px_10px_0px_rgba(0,0,0,0.1),inset_-4px_-4px_10px_0px_rgba(255,255,255,0.9)] transition-all duration-300";
  ...
  const activityLevels = [
    ...
    {
      value: "veryActive",
      label: "Very Active (Athletic/Physical Job)",
      multiplier: 1.9,
    },
  ];
  ...
  const calculateTDEE = (bmr) => {
    const activity = activityLevels.find(
      (a) => a.value === formData.activityLevel
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        "An error occurred while calculating your plan. Please try again."
      );
          ...
          <form onSubmit={handleSubmit}>
            ...
            <AnimatePresence>
              {error && (
                ...
                >
                  <div
                    className="relative px-4 py-3 text-red-700 bg-red-100 rounded-lg border border-red-400"
                    role="alert"
                  >
              ...
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                ...
                >
                  ...
                  <p className="text-2xl font-bold">
                    {results.calories.toLocaleString()} kcal
                  </p>
                </motion.div>
                <motion.div
                  whileHover={{ y: -5 }}
              ...
              >
                ...
                <div className="prose prose-purple max-w-none overflow-y-auto max-h-[500px] pr-4 bg-white/50 rounded-lg p-6 shadow-inner">
                  <ReactMarkdown
                    components={{
                      h1: ({ children }) => (
                        <h2 className="pb-2 mt-8 mb-4 text-2xl font-bold text-purple-700 border-b-2 border-purple-200">
                          {children}
                        </h2>
                      ),
                      h2: ({ children }) => (
                        <h3 className="mt-6 mb-3 text-xl font-semibold text-purple-600">
                          {children}
                        </h3>
                      ),
                      p: ({ children }) => (
                      ...
                      ul: ({ children }) => (
                        <ul className="grid gap-3 my-4 md:grid-cols-2">
                          {children}
                        </ul>
                      ),
                      li: ({ children }) => (
