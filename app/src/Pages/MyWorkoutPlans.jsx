    ...
    try {
      ...
      setError(
        error.response?.data?.message ||
          "Failed to load your workout plans. Please try again later."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDeletePlan = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        ...
        setWorkoutPlans(
          workoutPlans.filter((plan) => plan._id !== planToDelete)
        );
        setDeleteModalOpen(false);
        setPlanToDelete(null);
      } else {
        setError(
          error.response?.data?.message ||
            "Failed to delete the workout plan. Please try again later."
        );
      }
    }
  };
      ...
      <div className="relative text-white bg-gradient-to-r from-purple-600 to-purple-400">
        ...
        <div className="container relative z-10 px-4 py-8 mx-auto sm:py-16">
          <motion.div
            ...
            className="max-w-3xl"
          >
            <h1 className="mb-2 text-2xl font-bold sm:text-3xl md:text-4xl lg:text-5xl sm:mb-4">
              My Workout Plans
            </h1>
      ...
      <div className="container px-4 py-6 mx-auto sm:py-8 md:py-12">
        ...
        {loading ? (
          ...
          >
            ...
            <h2 className="mb-2 text-lg font-semibold text-gray-800 sm:text-xl md:text-2xl sm:mb-3">
              No Workout Plans Yet
            </h2>
            <p className="mx-auto mb-4 max-w-md text-xs text-gray-600 sm:text-sm md:text-base sm:mb-6 md:mb-8">
              Create your first custom workout plan to start tracking your
              fitness journey and achieving your goals.
            </p>
            <button
              onClick={navigateToCreatePlan}
            ...
            <div className="flex flex-col gap-3 mb-4 sm:flex-row sm:justify-between sm:items-center sm:gap-4 sm:mb-6 md:mb-8">
              <div>
                ...
                <p className="text-xs text-gray-600 sm:text-sm md:text-base">
                  You have {workoutPlans.length} custom workout{" "}
                  {workoutPlans.length === 1 ? "plan" : "plans"}
                </p>
              </div>
              <button
                onClick={navigateToCreatePlan}
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 sm:gap-4 md:gap-6">
              <AnimatePresence>
                {workoutPlans.map((plan, index) => (
                  ...
                  >
                    <div
                      className={`h-1.5 sm:h-2 md:h-3 bg-gradient-to-r ${getRandomGradient()}`}
                    ></div>
                    <div className="p-3 sm:p-4 md:p-6">
                      <div className="flex justify-between items-start mb-2 sm:mb-3 md:mb-4">
                        <h3 className="text-base font-semibold text-gray-900 transition-colors sm:text-lg md:text-xl group-hover:text-purple-600 line-clamp-1">
                          {plan.name}
                        </h3>
                      ...
                      {plan.description && (
                        <p className="mb-2 text-xs text-gray-600 sm:text-sm sm:mb-3 md:mb-4 line-clamp-2">
                          {plan.description}
                        </p>
                      )}
                      ...
                      <div className="flex items-center mb-2 text-xs text-gray-500 sm:mb-3 md:mb-4 sm:text-sm">
                        ...
                        <span>
                          {plan.days.length}{" "}
                          {plan.days.length === 1 ? "day" : "days"} workout
                        </span>
                      ...
                      <div className="flex flex-wrap gap-1 sm:gap-1.5 md:gap-2 mb-2 sm:mb-3 md:mb-4">
                        {plan.days.map((day, index) => (
                          <span
                            key={index}
                            className="px-1 sm:px-1.5 py-0.5 bg-gray-100 text-gray-700 text-xs rounded-md"
                          >
                            {day.name}
                          </span>
                        ))}
